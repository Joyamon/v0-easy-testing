        ...
        try:
            ...
            if scheduled_task.schedule_type == 'once':
                ...
                if scheduled_task.weekday and scheduled_task.scheduled_time:
                    ...
                    logger.info(f"创建每周任务: {task_name}, 星期{scheduled_task.weekday}, 时间: {scheduled_task.scheduled_time}")
                ...
                if scheduled_task.day_of_month and scheduled_task.scheduled_time:
                    ...
                    logger.info(f"创建每月任务: {task_name}, 日期: {scheduled_task.day_of_month}, 时间: {scheduled_task.scheduled_time}")
    ...
    def delete_celery_task(scheduled_task):
        ...
        if scheduled_task.celery_task_id:
            try:
                ...
                logger.error(f"删除Celery定时任务失败: {str(e)}")
                raise e
            
            # 清空celery_task_id（如果任务还存在的话）
            if hasattr(scheduled_task, 'pk') and scheduled_task.pk:
                try:
                    scheduled_task.celery_task_id = ''
                    scheduled_task.save(update_fields=['celery_task_id'])
                    logger.info(f"已清空任务的celery_task_id: {scheduled_task.name}")
                except Exception as e:
                    logger.warning(f"清空celery_task_id失败: {str(e)}")

    @staticmethod
    def delete_multiple_celery_tasks(scheduled_tasks):
        """批量删除Celery定时任务"""
        import logging
        logger = logging.getLogger(__name__)
        ...
        for task in scheduled_tasks:
            try:
                ...
                logger.error(f"批量删除失败 - 任务: {task.name}, 错误: {str(e)}")
                error_count += 1
        
        logger.info(f"批量删除完成 - 成功: {success_count}, 失败: {error_count}")
        return success_count, error_count

    @staticmethod
    def force_cleanup_task(task_id):
        """强制清理指定任务的所有Celery Beat记录"""
        import logging
        logger = logging.getLogger(__name__)
        
        try:
            # 查找所有可能相关的Celery任务
            related_tasks = PeriodicTask.objects.filter(
                name__startswith=f"scheduled_task_{task_id}_"
            )
            
            deleted_count = 0
            for celery_task in related_tasks:
                celery_task.delete()
                deleted_count += 1
                logger.info(f"强制删除Celery任务: {celery_task.name}")
        
        logger.info(f"强制清理完成，删除了 {deleted_count} 个Celery任务")
