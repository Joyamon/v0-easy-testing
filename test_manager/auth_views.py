...
def profile_view(request):
    """用户个人资料视图"""
    # 获取用户统计数据
    from test_manager.models import Project, TestCase, TestRun
    
    context = {
        'projects_count': Project.objects.filter(created_by=request.user).count(),
        'test_cases_count': TestCase.objects.filter(created_by=request.user).count(),
...
def social_connections_view(request):
    ...
    return render(request, 'auth/social_connections.html', {
        'github_account': github_account,
        'google_account': google_account,
    })
